...
//mem_size Kbytes
double memory_read_bandwidth(long long mem_size, long long step, long long times){
    mem_size <<= 10;
...
//mem_size Kbytes
double memory_write_bandwidth(long long mem_size, long long step, long long times){
    mem_size <<= 10;
    char *mem = (char*)malloc(sizeof(char) * mem_size);
    long long start = my_rdtsc();
...
double memory_pagefault(int mem_size, int times){
    ...
    for (int i = 0; i < times; i++) {
        ...
        char *mem = (char*)mmap(0, mem_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
